...
### Extension skills (Awesome UX pack)

These skills extend Intent with AI product design and tactical craft frameworks:

**AI product design**
- `/ai-governors` — Human-in-the-loop patterns (action plans, verification, undo, cost, memory, citations)
- `/ai-identifiers` — AI brand identity (name, avatar, color, personality)
- `/ai-inputs` — AI input patterns (open text, madlibs, autofill, voice, templates)
- `/ai-trust-builders` — Trust signals (caveats, consent, disclosure, watermarks)
- `/ai-tuners` — User-configurable AI behavior (modes, parameters, filters)
- `/ai-wayfinders` — Blank-slate onboarding (example prompts, galleries)

**Tactical craft & process**
- `/craft` — 12 visual-craft rules for polished UI (not flow-level UX)
- `/cognitive-load-conversion` — Cut extraneous load in forms, flows, layouts
- `/double-diamond` — Discover → Define → Develop → Deliver process guide
- `/empathy-mapping` — Says / Thinks / Does / Feels maps from research
- `/ux-personas` — Research-based persona construction
- `/ux-storyboard` — Panel-by-panel scenario storyboards
- `/feature-prioritization` — Weighted 2D prioritization matrix
- `/like-wish-what-if` — d.school critique / retrospective framework
- `/general-design-review` — Compact multi-framework design review
- `/persuasive-ux` — Fogg persuasion tools — **always cross-check against the anti-pattern catalog**

**Routing for AI product work**
- "How do users control the AI?" → `/ai-governors`
- "What should the AI be called / look like?" → `/ai-identifiers`
- "How do users talk to the AI?" → `/ai-inputs`
- "Users don't trust the AI" → `/ai-trust-builders` + `/ai-governors`
- "Blank slate anxiety / onboarding" → `/ai-wayfinders`
- "Polish the UI surface" → `/craft`
- "Forms feel heavy / conversion drop" → `/cognitive-load-conversion`
